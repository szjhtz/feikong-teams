...
服务提供 `GET /live`（进程存活）和 `GET /ready`（核心运行时就绪）探针，原有 `GET /health` 保持兼容。会话资源可通过 `PATCH /api/fkteams/sessions/:sessionID` 一次更新标题、收藏状态或当前智能体；旧的会话操作端点继续兼容。API 失败响应会在原有 `code`、`message` 之外返回稳定的 `error_code`，便于客户端按错误类型处理。
...
- 会话资源管理通过应用服务和存储端口复用，HTTP 层只负责协议转换；SkillHub 等外部 HTTP 能力由 adapter 实现并在 bootstrap 中显式组装。
