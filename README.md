...
- Web、CLI、SSE、WebSocket 和通道入口共用同一执行管线；WebSocket、流式任务和终端交互模式支持运行中 follow-up 排队和 steering 转向，steering 会在工具完成后的下一次模型调用前注入。Web 输入区提供运行中队列面板，可编辑、删除、上移/下移未执行的 steering/follow-up；终端运行中可用 `/queue` 管理未消费的转向队列。
