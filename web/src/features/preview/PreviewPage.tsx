    ...
    <div className="h-screen bg-muted/30 p-4">
      <Panel className="mx-auto flex h-full max-w-6xl flex-col">
        <PanelHeader className="flex items-center justify-between">
          ...
          <Button onClick={() => window.open(`/api/fkteams/preview/${encodeURIComponent(linkID)}`, "_blank")}>下载</Button>
        </PanelHeader>
        <PanelBody className="min-h-0 flex-1 p-0">
          <iframe className="h-full w-full border-0" src={`/api/fkteams/preview/${encodeURIComponent(linkID)}`} title="文件预览" />
        </PanelBody>
      </Panel>
    </div>
  );
