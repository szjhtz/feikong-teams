...
下载工作区中的指定文件或目录。文件直接作为附件返回；目录会自动打包为 ZIP。
...
文件内容以 `Content-Disposition: attachment` 方式返回；目录以 `application/zip` 流式返回，文件名为 `<目录名>.zip`。
...
文件内容通过 `http.ServeContent` 直接返回，浏览器会按内容类型直接展示。若目标为目录，会尝试返回该目录下的 `index.html`。
