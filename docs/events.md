...
- 助手实时输出拆为 `assistant_reasoning_delta` 和 `assistant_text_delta`；增量载荷只使用 `content`。历史 transcript 不保存这些增量事件，只在 `assistant_completed` 对应的聚合消息边界保存完整文本。
...
- `at`: 记录提交时间。
- `turn`: 会话内回合序号。运行时长 `turn_id` 只在 API 投影时生成，不落盘。
- `type`: transcript 记录类型，如 `user_message`、`assistant_message`、`tool_call_start`、`tool_call_end`。
- `agent`: 记录所属智能体。用户消息不写 `agent:"user"`。
- `content`、`reasoning`、`usage`、`tool_call`、`ask`、`error`: 按记录类型出现的结构化事实字段，不再包一层通用 `payload`。
- `tool_call_id`、`parent_tool_call_id`、`agent_run_id`: 工具和子智能体轨迹所需的结构化关联字段。
