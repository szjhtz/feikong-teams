...
- `internal/adapters/runtime/eino` 核心适配层：顶层 agent/model/tool/runner/engine 转换代码已直接使用 `internal/ports/runtime`、`internal/domain/message`、`internal/domain/event` 与 `internal/runtime/checkpoint`，禁止再通过 `agentcore` 旧门面中转；`middlewares/*` 单独按中间件边界继续迁移。
